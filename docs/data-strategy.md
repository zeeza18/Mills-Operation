...
I decided on five signals per roll stand, sampled every minute: vibration, bearing temperature, motor current, line speed, and coolant pressure. I didn't pick these because they were easy to fake — I picked them because between them they cover three different failure signatures (mechanical, thermal, electrical), one operational-response signal, and one that moves in the *opposite* direction from the rest. I've made the mistake before of loading a model up with features that are all correlated and teach it nothing new — wanted to avoid doing that again here.

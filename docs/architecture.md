...
- It doesn't do automated shutdown or control-system intervention. It flags and explains — a human decides. I don't think an AI system should ever be the one pulling a mill offline on its own, and I say more about why in the security & risk doc.
...
There's also a testing/CI layer that isn't in the diagram above because it sits around the system, not inside it: `tests/dashboard.spec.js` is a Playwright suite covering the dashboard's core flows (loads, shows real detection numbers, renders per-stand charts, the copilot button produces a grounded response), wired into GitHub Actions (`.github/workflows/ci.yml`) — the accessible stand-in for what would be an Azure DevOps pipeline at Nucor, gating merges the same way. It already earned its keep once during this build: it caught a real `ModuleNotFoundError` in the dashboard that my own manual browser testing had missed, because the test suite launches the app exactly the way the README tells people to (`streamlit run app/dashboard.py`), and my manual testing had accidentally used a different invocation (`python -m streamlit run`) that happened to sidestep the bug. Full story in `docs/ai-partnership-log.md`.
...
- **Real-time streaming (Kafka/similar) instead of batch scoring.** Rejected for the prototype — it's the right architecture eventually, but building streaming infra to process a synthetic CSV would have been effort spent on plumbing instead of on the actual detection problem. Documented as a known gap, not silently ignored.

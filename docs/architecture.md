...
There's also a testing and CI layer that isn't in the diagram above because it sits around the system, not inside it. `tests/dashboard.spec.js` is a Playwright suite covering the frontend's core flows (loads, shows real detection numbers, renders per-stand charts, the copilot button produces a grounded response), wired into GitHub Actions (`.github/workflows/ci.yml`), the accessible stand-in for what would be an Azure DevOps pipeline at Nucor, gating merges the same way. It's already earned its keep twice during this build. Early on, when the UI was a Streamlit prototype, it caught a real `ModuleNotFoundError` that my own manual browser testing had missed, because the test suite launched the app exactly the way the README told people to, while my manual testing had accidentally used a different invocation that happened to sidestep the bug. Later, after rebuilding the UI in React, it caught a timing bug: testing against Vite's dev server (which cold-bundles heavy dependencies on first load) produced flaky failures that had nothing to do with the app itself, fixed by testing the production build instead. Full story in `docs/ai-partnership-log.md`.
